...
public class Electre4 {
	...
	public Electre4(Data data, Double p, Double q, Double nv) {
	...
	private Boolean is_alternative_preferred(int b, int a, int criterium) {
		
		if (criteria_.get(criterium).direction == Direction.MAX) 
			return (alternatives_.get(a)[criterium]*(1+nv_) >= alternatives_.get(b)[criterium]);
		
		else return (alternatives_.get(a)[criterium]*(1-nv_) <= alternatives_.get(b)[criterium]);
